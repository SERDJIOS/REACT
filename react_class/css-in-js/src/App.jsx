...
const dynamicStyle = (active) => css`
  background: ${active ? 'linear-gradient(45deg, #FE6B8B 30%, #FF8EB3 90%)' : 'grey'};
  border: 0;
  border-radius: 3;
  box-shadow: 0 3px 5px 2px ${active ? 'rgba(255, 105, 135, .3)' : 'rgba(0, 0, 0, .3)'};
  ...
  return (
    <ThemeProvider theme={isDarkMode ? darkTheme : lightTheme}>
      ...
      <div style={{ padding: 20 }}>
        ...
        <button css={dynamicStyle(active)} onClick={handleClick}>Emotion button</button>
      </div>
      <div style={{ padding: 20 }}>
        <Typography variant='h2'>THEME</Typography>
        <Button variant="contained" color="primary" style={{ marginLeft: 10 }}>Primary</Button>
        <Button variant="contained" color="secondary" style={{ marginLeft: 10 }}>Secondary</Button>
      </div>
    </ThemeProvider>
  );
}